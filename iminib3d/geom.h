...
class MMatrix{
	static MMatrix tmps[64];
	static MMatrix &alloc_tmp(){ static int tmp=0;return tmps[tmp++&63];	}
	friend class Transform;
public:
	Vector i,j,k;

	MMatrix():i(Vector(1,0,0)),j(Vector(0,1,0)),k(Vector(0,0,1)){
	}
	MMatrix( const Vector &i,const Vector &j,const Vector &k ):i(i),j(j),k(k){
	}
	MMatrix( const Quat &q ){
	...
	MMatrix( float angle,const Vector &axis ){
		const Vector &u=axis;
	...
	bool operator==( const MMatrix &q )const{
	...
	MMatrix &operator*( const MMatrix &q )const{
		MMatrix &m=alloc_tmp();
...
public:
	...
	Transform( const MMatrix &m ):m(m){
	}
	Transform( const Vector &v ):v(v){
	}
	Transform( const MMatrix &m,const Vector &v ):m(m),v(v){
	}
...
inline float transformRadius( float r,const MMatrix &t ){
	static const float sq_3=sqrtf(1.0f/3.0f);
	return (t * Vector( sq_3,sq_3,sq_3 )).length()*r;
}

inline MMatrix pitchMMatrix( float q ){
	return MMatrix( Vector(1,0,0),Vector(0,cosf(q),sinf(q)),Vector(0,-sinf(q),cosf(q)) );
}

inline MMatrix yawMMatrix( float q ){
	return MMatrix( Vector(cosf(q),0,sinf(q)),Vector(0,1,0),Vector(-sinf(q),0,cosf(q)) );
}

inline MMatrix rollMMatrix( float q ){
	return MMatrix( Vector(cosf(q),sinf(q),0),Vector(-sinf(q),cosf(q),0),Vector(0,0,1) );
}

inline float MMatrixPitch( const MMatrix &m ){
	return m.k.pitch();
//	return asinf( -m.k.y );
}

inline float MMatrixYaw( const MMatrix &m ){
	return m.k.yaw();
	//return atan2f( -m.k.x,m.k.z );
}

inline float MMatrixRoll( const MMatrix &m ){
	return atan2f( m.i.y,m.j.y );
	//MMatrix t=pitchMMatrix( -MMatrixPitch(m) )*yawMMatrix( -MMatrixYaw(m) )*m;
	//return atan2f( t.i.y,t.i.x );
}

inline MMatrix scaleMMatrix( float x,float y,float z ){
	return MMatrix( Vector( x,0,0 ),Vector( 0,y,0 ),Vector( 0,0,z ) );
}

inline MMatrix scaleMMatrix( const Vector &scale ){
	return MMatrix( Vector( scale.x,0,0 ),Vector( 0,scale.y,0 ),Vector( 0,0,scale.z ) );
}
...
inline MMatrix rotationMMatrix( float p,float y,float r ){
	return yawMMatrix(y)*pitchMMatrix(p)*rollMMatrix(r);
}

inline MMatrix rotationMMatrix( const Vector &rot ){
	return yawMMatrix(rot.y)*pitchMMatrix(rot.x)*rollMMatrix(rot.z);
}
...
//	return atan2f( i.y,j.y );
	return MMatrixRoll( q );
}

inline Quat MMatrixQuat( const MMatrix &p ){
	MMatrix m=p;
	m.orthogonalize();
