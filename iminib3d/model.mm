...
Mesh* LoadAnimB3DFile(string f_name,Entity* parent_ent_ext){
	...
	*/
	...
	do{
		...
		if(tag_id==TEXS){
			...
			while(NewTag(new_tag)!=true && file->Eof()!=true){
			...
			int vf1=v_flags&1;
			if((vf1==0) && (new_tag!="TRIS")) mesh->UpdateNormals(); // if no normal data supplied and no further tri data then update normals
	...
	//cout << f_name << " loaded in " << ms << "ms" << endl;
...
Mesh* LoadAnimB3D(string f_name,Entity* parent_ent_ext){
	...
	*/
	...
	tag=ReadTag(bank,offset);
	bank->PeekLong(offset);offset+=8;
	int vno=bank->PeekInt(offset);offset+=4;
	if(tag!="BB3D") RuntimeError(tag.c_str());//"Invalid b3d file");
	if((vno/100)>0) RuntimeError("Invalid b3d file version");
	...
	do{
		...
		if(NewTag(new_tag)==true){
			...
			prev_tag=tag;
			tag=new_tag;
			bank->PeekInt(offset);offset+=4;

			size=bank->PeekInt(offset);offset+=4;
			...
			if(tag=="NODE"){
				...
				if(node_level>0){
					...
					int fd=0;
					do{
						fd=offset-node_pos[node_level-1];
						if(fd>=8){
				...
				node_pos[node_level]=offset+size;
		...
		if(tag_id==TEXS){
			...
			while(NewTag(new_tag)!=true && offset<bank->size){
				...
				te_file=b3dReadString(bank,offset);

				te_flags=bank->PeekInt(offset);offset+=4;
				te_blend=bank->PeekInt(offset);offset+=4;
				te_u_pos=bank->PeekFloat(offset);offset+=4;
				te_v_pos=bank->PeekFloat(offset);offset+=4;
				te_u_scale=bank->PeekFloat(offset);offset+=4;
				te_v_scale=bank->PeekFloat(offset);offset+=4;
				te_angle=bank->PeekFloat(offset);offset+=4;
				...
				*/
				...
				//cout << "LoadTexture ms: " << Millisecs()-tex_ms << endl;
			...
			b_no_texs=bank->PeekInt(offset);offset+=4;

			new_tag=ReadTag(bank,offset);

			while((NewTag(new_tag)!=true) && (offset<bank->size)){

				b_name=b3dReadString(bank,offset);

				b_red=bank->PeekFloat(offset);offset+=4;
				b_green=bank->PeekFloat(offset);offset+=4;
				b_blue=bank->PeekFloat(offset);offset+=4;
				b_alpha=bank->PeekFloat(offset);offset+=4;
				b_shine=bank->PeekFloat(offset);offset+=4;
				b_blend=bank->PeekInt(offset);offset+=4;
				b_fx=bank->PeekInt(offset);offset+=4;
				...
				for(int ix=0;ix<b_no_texs;ix++){
					...
					b_tex_id=bank->PeekInt(offset);offset+=4;
			...
			n_name=b3dReadString(bank,offset);
			n_px=bank->PeekFloat(offset);offset+=4;
			n_py=bank->PeekFloat(offset);offset+=4;
			n_pz=bank->PeekFloat(offset)*-1;offset+=4;
			n_sx=bank->PeekFloat(offset);offset+=4;
			n_sy=bank->PeekFloat(offset);offset+=4;
			n_sz=bank->PeekFloat(offset);offset+=4;
			n_qw=bank->PeekFloat(offset);offset+=4;
			n_qx=bank->PeekFloat(offset);offset+=4;
			n_qy=bank->PeekFloat(offset);offset+=4;
			n_qz=bank->PeekFloat(offset);offset+=4;
			float pitch=0;
			float yaw=0;
			float roll=0;
			QuatToEuler(n_qw,n_qx,n_qy,-n_qz,pitch,yaw,roll);
			...
			m_brush_id=bank->PeekInt(offset);offset+=4;
			...
			v_mesh=new Mesh;
			v_surf=v_mesh->CreateSurface();
			v_flags=bank->PeekInt(offset);offset+=4;
			v_tc_sets=bank->PeekInt(offset);offset+=4;
			v_tc_size=bank->PeekInt(offset);offset+=4;
			v_sz=12+v_tc_sets*v_tc_size*4;
			if(v_flags & 1) v_sz=v_sz+12;
			if(v_flags & 2) v_sz=v_sz+16;
			...
			while((NewTag(new_tag)!=true) && (offset<bank->size)){
				...
				v_x=bank->PeekFloat(offset);offset+=4;
				v_y=bank->PeekFloat(offset);offset+=4;
				v_z=bank->PeekFloat(offset);offset+=4;
				
				if(v_flags&1){
					v_nx=bank->PeekFloat(offset);offset+=4;
					v_ny=bank->PeekFloat(offset);offset+=4;
					v_nz=bank->PeekFloat(offset);offset+=4;
				}
				
				if(v_flags&2){
					v_r=bank->PeekFloat(offset)*255.0;offset+=4; // *255 as VertexColor requires 0-255 values
					v_g=bank->PeekFloat(offset)*255.0;offset+=4;
					v_b=bank->PeekFloat(offset)*255.0;offset+=4;
					v_a=bank->PeekFloat(offset);offset+=4;
				}
				...
				for(int j=0;j<v_tc_sets;j++){ // texture coords per vertex - 1 for simple uv, 8 max
					for(int k=1;k<v_tc_size+1;k++){ // components per set - 2 for simple uv, 4 max
						if(k==1) { v_u=bank->PeekFloat(offset);offset+=4; }
						if(k==2) { v_v=bank->PeekFloat(offset);offset+=4; }
						if(k==3) { v_w=bank->PeekFloat(offset);offset+=4; }
					}
					if(j==0 || j==1) v_surf->VertexTexCoords(v_id,v_u,v_v,v_w,j);
				}
			...
			//cout << "Vertices ms: " << Millisecs()-vert_ms << endl;
			
		}else if(tag_id==TRIS){

			int old_tr_brush_id=tr_brush_id;
			tr_brush_id=bank->PeekInt(offset);offset+=4;
			...
			while(NewTag(new_tag)!=true && offset<bank->size){
				...
				tr_vid0=bank->PeekInt(offset);offset+=4;
				tr_vid1=bank->PeekInt(offset);offset+=4;
				tr_vid2=bank->PeekInt(offset);offset+=4;
			...
			a_flags=bank->PeekInt(offset);offset+=4;
			a_frames=bank->PeekInt(offset);offset+=4;
			a_fps=bank->PeekFloat(offset);offset+=4;
			...
			while(NewTag(new_tag)!=true && offset<bank->size){
				...
				bo_vert_id=bank->PeekInt(offset);offset+=4;
				bo_vert_w=bank->PeekFloat(offset);offset+=4;
			...
			k_flags=bank->PeekInt(offset);offset+=4;
		
			new_tag=ReadTag(bank,offset);

			while(NewTag(new_tag)!=true && offset<bank->size){
		
				k_frame=bank->PeekInt(offset);offset+=4;
				
				if(k_flags&1){
					k_px=bank->PeekFloat(offset);offset+=4;
					k_py=bank->PeekFloat(offset);offset+=4;
					k_pz=-(bank->PeekFloat(offset));offset+=4;
				}
				if(k_flags&2){
					k_sx=bank->PeekFloat(offset);offset+=4;
					k_sy=bank->PeekFloat(offset);offset+=4;
					k_sz=bank->PeekFloat(offset);offset+=4;
				}
				if(k_flags&4){
					k_qw=-(bank->PeekFloat(offset));offset+=4;
					k_qx=bank->PeekFloat(offset);offset+=4;
					k_qy=bank->PeekFloat(offset);offset+=4;
					k_qz=-(bank->PeekFloat(offset));offset+=4;
				}
...
string b3dReadString(Bank* bank,int &offset){
	string t="";
	for(;;){
		char ch=bank->PeekByte(offset);offset+=1;
		if(ch==0){
			return t;
		}
		t=t+ch;
...
string ReadTag(Bank* bank,int offset){
	...
	for(int i=0;i<4;i++){
		...
		char rb=bank->PeekByte(offset);offset+=1;
